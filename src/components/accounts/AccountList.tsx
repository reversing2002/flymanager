...
const AccountList = () => {
  ...
  const filteredEntries = entries.filter((entry) => {
    ...
    if (filters.validated !== "all") {
      if (filters.validated === "yes" && !entry.is_validated) return false;
      if (filters.validated === "no" && entry.is_validated) return false;
    }
    ...
    if (
      ...
      entry.assigned_to_id !== filters.assignedToId
    )
      return false;
    return true;
  });
  ...
  const validatedBalance = filteredEntries
              ...
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${
                    validatedBalance >= 0 ? "text-emerald-600" : "text-red-600"
                  }`}
                >
            ...
            <tbody>
              {filteredEntries.map((entry) => (
                ...
                >
                  ...
                  <td className="p-4">{getTypeLabel(entry.type)}</td>
                  <td
                  ...
                  <td className="p-4">{getUserName(entry.assigned_to_id)}</td>
                  <td className="p-4 text-center">
                    <button
