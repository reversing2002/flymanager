...
const ACCOUNTING_CATEGORIES = [
  { id: 'REGULAR', label: 'Vol régulier' },
  { id: 'INSTRUCTION', label: 'Instruction' },
  { id: 'INITIATION', label: 'Initiation' },
  { id: 'DISCOVERY', label: 'Vol découverte' },
  { id: 'BIA', label: 'BIA' },
  { id: 'FERRY', label: 'Convoyage' }
];
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {ACCOUNTING_CATEGORIES.map((category) => (
              <label key={category.id} className="flex items-center">
                <input
                  type="checkbox"
                  checked={filters.accountingCategories.includes(category.id)}
                  onChange={(e) => {
                    const newCategories = e.target.checked
                      ? [...filters.accountingCategories, category.id]
                      : filters.accountingCategories.filter((c) => c !== category.id);
